...
cross_validate_response=$(curl -s -w "%{http_code}" -X POST -H "Content-Type: application/x-yaml" -d "$cross_validate_yaml" "$TRAINING_RASA/model/test/intents?cross_validation_folds=1")
