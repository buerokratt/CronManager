...
processing_res=$(curl "$TRAINING_PUBLIC_RUUTER/rasa/model/add-new-model-processing")
...
if [ "$train_status" != "200" ]; then
    ...
    error_res=$(curl "$TRAINING_PUBLIC_RUUTER/rasa/model/add-new-model-error")
...
testing_res=$(curl "$TRAINING_PUBLIC_RUUTER/rasa/model/add-new-model-testing")
...
cv_res=$(curl "$TRAINING_PUBLIC_RUUTER/rasa/model/add-new-model-cross-validating")
...
ready_res=$(curl -X POST -H "Content-Type: application/json" --data-binary @temp3 "$TRAINING_PUBLIC_RUUTER/rasa/model/add-new-model-ready") || echo "failed to send to ruuter"
